...
- 💾 Esportazione/importazione dello stato in JSON (versione 2.4; le importazioni continuano ad accettare le versioni legacy 1.4, 2.0, 2.1, 2.2 e 2.3 e vengono migrate automaticamente, con stati personalizzati dei plugin e note per dente)
...
- 🧪 563 test automatizzati (Vitest) in 55 file di test che coprono numerazione, traduzioni, preset, i18n, componente App, tema, touch, plugin, accessibilità e parità degli assi clinici/diagnostici
...
**Gravità della carie** (`cariesSeverity`; campo unificato per superficie, `0`–`6`): su una superficie senza otturazione viene letta come scala di profondità ICDAS (`superficial` / `dentin` / `deep`, oppure i codici ICDAS II grezzi `0–6` quando `enableIcdas` è attivo) e visualizza il livello primario `caries-{surface}`; su una superficie con otturazione viene letta come punteggio CARS con nome (`0` sano … `6` cavità estesa) e visualizza invece il livello `subcaries-{surface}` (carie secondaria) — una superficie non è mai primaria e ricorrente allo stesso tempo
...
### 🧪 Test
```bash
npm run test           # Esegui tutti i 563 test
...
L'esportazione crea un file JSON (versione `2.4`; le importazioni accettano anche le versioni legacy `1.4`, `2.0`, `2.1`, `2.2` e `2.3` e vengono migrate automaticamente) contenente:
...
- `src/__tests__/` - suite di test Vitest (563 test in 55 file)
