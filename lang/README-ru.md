...
- 💾 Экспорт/импорт статуса в JSON (версия 2.3; импорт по-прежнему принимает версии 1.4, 2.0, 2.1 и 2.2 и автоматически мигрирует их, с пользовательскими состояниями плагинов и заметками к зубам)
...
- 🦷 Вторичный (рецидивирующий) кариес: сохраняемая оценка CARS по поверхностям (0–6, `secondaryCaries`), отображаемая как непрозрачность слоя кариеса — заменяет прежнее определение по пересечению кариеса и пломбы (мигрированное устаревшее состояние получает каноническую оценку «умеренная»)
- 🌱 Кариес корня (`rootCaries`: none / active / arrested / active-cavitated), задействующий отдельный слой изображения кариеса корня
- 📡 Рентгенологическая глубина кариеса (`radiographicDepth`: none / E1 / E2 / D1 / D2 / D3 по поверхности), независимая от визуальной шкалы глубины ICDAS, отображается в виде значка и передаётся через собственное наблюдение FHIR
...
- 🧪 506 автоматизированных тестов (Vitest) в 50 файлах: нумерация, переводы, шаблоны, i18n, компонент App, тема, сенсорный ввод, плагины, доступность и паритет клинических/диагностических осей
...
**Кариес корня** (`rootCaries`; задействует слой изображения `caries-root` на присутствующем зубе):
`none`, `active`, `arrested`, `active-cavitated`

**Оценка вторичного (рецидивирующего) кариеса** (`secondaryCaries`; сохраняется по поверхности, CARS `0` (здоров) – `6` (кавитация), отображается как непрозрачность слоя кариеса):
`0`, `1`, `2`, `3`, `4`, `5`, `6`

**Рентгенологическая глубина кариеса** (`radiographicDepth`; по поверхности, независима от визуальной шкалы ICDAS/`cariesDepths`):
`none`, `E1`, `E2`, `D1`, `D2`, `D3`
...
### 🧪 Тестирование
```bash
npm run test           # Запустить все 506 тестов
...
При экспорте создаётся файл JSON (версия `2.3`; импорт также принимает устаревшие версии `1.4`, `2.0`, `2.1` и `2.2` и автоматически мигрирует их), содержащий:
...
- `secondaryCaries` — оценка вторичного кариеса CARS по поверхностям (0-6)
- `radiographicDepth` — рентгенологическая глубина кариеса по поверхностям (none/E1/E2/D1/D2/D3), независимая от визуальной шкалы ICDAS
...
- `src/__tests__/` — набор тестов Vitest (506 тестов в 50 файлах)
