...
- 💽 حفظ اختياري للحالة في localStorage (‏`enablePersistence`/‏`disablePersistence`/‏`clearPersistedState`/‏`isPersistenceEnabled`) — معطّل افتراضيًا؛ يحفظ مخطط الحالة تلقائيًا (واختياريًا مخطط الخطة أيضًا) عند كل تغيير في الحالة، ويستعيده عند إعادة تحميل المكوّن في المرة التالية، مع حد أقصى للحجم قدره 4 ميغابايت وتمرير أخطاء التخزين/التحليل إلى دالة استدعاء `onError` (أو `console.warn`) بدلًا من رمي استثناء
- 🔗 تصدير HL7 FHIR R4 (حزمة تجميعية (Bundle) من ملاحظات (Observations) لكل سن، مع ترميز أسنان وفق ISO 3950 للتسنين الدائم **و**اللبني (51-85، مع استيراد يعيد التخطيط دون فقدان بيانات)، ونظام ترميز محلي — مع التخطيط لدعم ترميز SNOMED CT مستقبلًا)؛ كما يحمل مكوّن النخر (caries) الذي له شدة مسجَّلة ترميزًا إضافيًا لنظام التقييم — ICDAS على سطح أساسي (غير محشو)، وCARS على سطح ناكس (محشو)
...
- 🛡️ سياسة أمان المحتوى (CSP): يُدرج إصدار الإنتاج (production build) الخاص بالنسخة التجريبية وسم CSP تلقائيًا (خادم التطوير غير متأثر) — ينبغي على التطبيقات المضيفة التي تُضمّن المكوّن ضبط سياسة CSP الخاصة بها
...
- 🧪 مجموعة اختبارات Vitest آلية شاملة تغطي الترقيم، والترجمات، والأنماط الجاهزة، والدعم متعدد اللغات، ومكوّن App، والسمة، واللمس، والإضافات، وإمكانية الوصول، وتكافؤ محاور التشخيص/المحاور السريرية
...
- 🌐 الترجمة والتوطين (HU/EN/DE/ES/IT/SK/PL/RU/PT-BR/AR/ZH/FR)
...
حفظ اختياري لحالة الحالة السريرية في `localStorage` (‏`src/persistence.ts`، يُعاد تصديره من نقطة دخول الحزمة). معطّل افتراضيًا — التكاملات القائمة لا تتأثر ما لم يقم التطبيق المضيف بتفعيله صراحةً، وينبغي استدعاؤه **بعد** أن يتم تركيب (mount) الأودونتوغرام (تعيد الاستعادة رسم DOM الحي عبر `importStatus()`):
...
enablePersistence({
  key: "my-app-odontogram",   // الافتراضي: "react-advanced-odontogram"
  includePlan: true,          // حفظ مخطط الخطة أيضًا؛ الافتراضي: false
...
| الدالة | الوصف |
|---|---|
| `enablePersistence(options?)` | تستعيد حالة محفوظة سابقًا (إن وُجدت) عبر `importStatus()`، ثم تحفظ مخطط الحالة إلى `localStorage` عند كل تغيير في الحالة. متكررة الاستدعاء بأمان (idempotent) — استدعاؤها مرة أخرى يستبدل الاشتراك/الخيارات السابقة. **يجب استدعاؤها بعد تركيب الأودونتوغرام.** |
| `disablePersistence()` | يوقف الحفظ؛ يبقى العنصر المخزَّن كما هو. |
...
|---|---|---|---|
| `key` | `string` | `"react-advanced-odontogram"` | مفتاح `localStorage`. |
...
ملاحظات: لا تتم قراءة أو كتابة أي شيء في `localStorage` ما لم يُستدعَ `enablePersistence()`؛ يُوقِف حد أقصى للحجم قدره 4 ميغابايت أي عملية حفظ متضخمة (مع الإبلاغ عنها عبر `onError`/‏`console.warn`) بدلًا من رمي استثناء؛ يتم التقاط كل فشل في التخزين أو JSON — تجاوز الحصة، إطار iframe مقيّد، بيانات مخزَّنة تالفة أو غير معروفة، إلخ — والإبلاغ عنه. هذه الوحدة لا ترمي استثناءً أبدًا.

ملاحظة: تفعيل الحفظ يستعيد الحالة المحفوظة عبر `importStatus()`، مما يستبدل الحالة الحالية — بما في ذلك مخطط خطة قيد التنفيذ إذا لم تحتوِ البيانات المحفوظة على أي خطة. فعّل الحفظ عند بدء التشغيل (مباشرة بعد تركيب الأودونتوغرام)، وليس في منتصف الجلسة.

ملاحظة: قد تتضمن البيانات المحفوظة معلومات تُعرّف هوية المريض (اسم المريض، تاريخ الفحص) بنص واضح في `localStorage`. إذا سجّلت مثل هذه البيانات ضمن الأودونتوغرام، فتأكد من توفير حماية على مستوى الجهاز أو امسحها باستخدام `clearPersistedState()` عند الاقتضاء.
...
- `src/i18n/` - الترجمات (HU/EN/DE/ES/IT/SK/PL/RU/PT-BR/AR/ZH/FR) وخطاف الترجمة
...
- `src/__tests__/` + `src/registry/__tests__/` - مجموعة اختبارات Vitest آلية شاملة
...
- **الإضافات تعمل ككود موثوق.** يتم حقن القيمة التي تُرجعها دالة `renderSvg()` الخاصة بالإضافة داخل SVG المخطط الحي. تُنقَّى تلك المخرجات باستخدام [DOMPurify](https://github.com/cure53/DOMPurify) (نمط SVG، بالإضافة إلى `svgFilters`) قبل الإدراج — يُمنع تمامًا كل من `<script>` و`<iframe>` و`<object>` و`<embed>` و`<foreignObject>`، ويتم إسقاط المخرجات الخبيثة بالكامل بدلًا من عرضها جزئيًا. يقلّل هذا من نطاق الضرر الناتج عن إضافة مخترَقة أو معطوبة، لكن يجب أن تظل الإضافات محمَّلة فقط من مصادر تثق بها — فالتنقية شبكة أمان، لا بديلًا عن التحقق.
- **سياسة أمان المحتوى (Content-Security-Policy).** يُدرج إصدار **الإنتاج (production build)** الخاص بالنسخة التجريبية هذه السياسة عبر وسم `<meta http-equiv="Content-Security-Policy">` (خادم التطوير غير متأثر):
  ...
  ينبغي على التطبيقات المضيفة التي تُضمِّن `OdontogramShell` ضبط سياسة CSP الخاصة بها بما يناسب بيئة النشر لديها — فالمكوّن لا يُدرج سياسة تلقائيًا عند استخدامه كمكتبة.
