...
- **Collapsible card state (session-only).** Five cards (Controls, Statuses,
  Caries/Fillings/Root/Periodontium) now track their collapsed/expanded state
  as a session-only UI flag (never serialized to the export payload, same
  convention as other UI-layout session flags). New public API:
